import BarView from "../src/bars/BarView.js";
  ...
  describe("barView methods", () => {
    ...
    describe("getStartXList", () => {
      test("should throw exception if durations is not array", () => {
        const fn = () => {
          barView.getStartXList("34", "not an array");
        };
        expect(fn).toThrow();
      });
      ...
      test("should return array of same length as durations", () => {
        ...
        expect(barView.getStartXList("44", durations)).toHaveLength(
          durations.length
        );
      });
    });
