...
const generateBars = (width, height, numColumns, numRows) => {
  let bars = [...Array(numColumns)].map(
    (item, i) => [...Array(numRows)].map(
      (item, j) => ({
        dimensions: {
          x: j * width,
          y: i * height
...
describe('Selections', () => {
  ...
  describe('getLineChange', () => {
    let firstLineChange
    beforeAll( () => {
      const pos1 = {x: bars[3].dimensions.x + BAR_WIDTH, y: bars[3].dimensions.y}
  ...
  describe('getRectsDims', () => {
    ...
    test('selections finishing in middle of bar', () => {
      ...
      expect(Selections.getRectsDims(bars, barView, startX, selection)).toEqual(selectionPos)
    })
    ...
    test('selections including two bar lines', () => {
      ...
      .toEqual([firstLineSelection, secondLineSelection])
    })
    test('selections including three bar lines', () => {
      ...
      expect(Selections.getRectsDims(bars, barView, startX, selection))
      .toEqual([ firstLineSelection, secondLineSelection, thirdLineSelection
      ])
    })
