...
describe("Endings", () => {
  ...
  test("should throw exception if one of two arguments is missing", () => {
    expect(() => {
      Endings(null, barView);
    }).toThrow("bars must be a non-empty array");
    expect(() => {
      Endings(fakeBars);
    }).toThrow("barView argument required");
  });
  ...
  test("should return correct structure", () => {
    const barsWithDimensions = aloneTogether.map((bar, i) =>
      barView.setDimensions(bar, i)
    );
