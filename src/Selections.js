const Selections = (() => {
  const getLineChange = (bars, barView, i) => {
    ...
    while (
      isNotLast(bars, i) &&
      bars[i].dimensions.y === bars[i + 1].dimensions.y
    )
      i++;
  ...
  const getRectsDims = (bars, barView, startX, selection) => {
    ...
    const endX =
      selection.end.chord === endBarChordsX.length - 1
        ? endBar.x + barView.bar.width
        : endBar.x + endBarChordsX[selection.end.chord + 1];
  ...
  const getRects = (bars, barView, startX, selections) => {
    ...
    for (let selection of selections) {
      rectsDims = getRectsDims(bars, barView, startX, selection).map(
        (dims, i) => {
